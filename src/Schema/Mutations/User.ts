...
const CreateUserReturnType = new GraphQLObjectType({
  name: "CreateUserReturnType",
  fields: () => ({
    userFirstName: { type: GraphQLString },
    userLastName: { type: GraphQLString },
    userId: { type: GraphQLInt },
    userEmail: { type: GraphQLString },
    userAccesToken: { type: GraphQLString }
  })
});
...
export const CREATE_USER = {
  ...
  resolve: async (parent: any, args: any) => {
    ...
    const token = jwt.sign(
      { userId: newUser.usr_id, usr_email: newUser.usr_email },
      JWT_SECRET,
      { expiresIn: '12h' } 
    );

    return {
      userFirstName: newUser.usr_first_name,
      userLastName: newUser.usr_last_name,
      userId: newUser.usr_id,
      userEmail: newUser.usr_email,
